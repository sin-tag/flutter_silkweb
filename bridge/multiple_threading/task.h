...
class ConcreteSyncTask : public SyncTask {
 public:
  ...
  void wait() override {
    ...
    future_.wait();
#else
    // Bumped to 5s; the prior 2s value silently expired during slow Dart
    // frames (e.g. heavy GC), and `future_.get()` later threw an
    // unhandled std::future_error inside InvokeBindingMethod.
    auto status = future_.wait_for(std::chrono::milliseconds(5000));
    if (status == std::future_status::timeout) {
      timed_out_ = true;
      WEBF_LOG(ERROR) << "SyncTask wait timed out after 5s — Dart frame stalled or deadlock";
    }
#endif
  }
