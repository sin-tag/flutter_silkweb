...
abstract class RenderBoxModel extends RenderBox
  ...
  void _paintFilteredContent(PaintingContext context, Offset offset,
    ...
    // Outermost wrapper: backdrop-filter operates on what is already painted
    // BELOW this layer (the parent's contents within our bounds), so it must
    // be pushed before our own filter chain runs.
    paintBackdropFilter(context, offset, _chainPaintColorFilter);
    if (skipDropShadow) {
      _paintingFilterContent = previous;
    }
  }

  void _chainPaintColorFilter(PaintingContext context, Offset offset) {
  ...
  void paintBackdropFilter(PaintingContext context, Offset offset,
      PaintingContextCallback callback) {
    final ImageFilter? filter = renderStyle.backdropImageFilter;
    if (filter != null) {
      _backdropFilterLayer.layer ??= BackdropFilterLayer();
      _backdropFilterLayer.layer!.filter = filter;
      context.pushLayer(_backdropFilterLayer.layer!, callback, offset);
    } else {
      callback(context, offset);
    }
  }
