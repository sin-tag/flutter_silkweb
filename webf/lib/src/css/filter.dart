...
mixin CSSFilterEffectsMixin on RenderStyle {
  ...
  set filter(List<CSSFunctionalNotation>? functions) {
    ...
    resetBoxDecoration();
    _propagateFilterChange();
  }

  // backdrop-filter — applies to the area BEHIND the element (Tailwind
  // backdrop-blur, backdrop-brightness, etc.). Reuses the same function
  // parsers as `filter`, but only the image-filter side has visual meaning;
  // color-mutating functions like grayscale on a backdrop also map through
  // `_parseImageFilters` because Skia treats them via ImageFilter.
  ImageFilter? _cachedBackdropImageFilter;
  List<CSSFunctionalNotation>? _backdropFilter;

  @override
  List<CSSFunctionalNotation>? get backdropFilter => _backdropFilter;

  @override
  ImageFilter? get backdropImageFilter {
    if (_backdropFilter == null) return null;
    return _cachedBackdropImageFilter ??= _parseImageFilters(_backdropFilter!);
  }

  set backdropFilter(List<CSSFunctionalNotation>? functions) {
    _backdropFilter = functions;
    _cachedBackdropImageFilter = null;
    markNeedsPaint();
    final RenderStyle? parentRenderStyle = getAttachedRenderParentRenderStyle();
    parentRenderStyle?.markChildrenNeedsSort();
  }

  void _propagateFilterChange() {
