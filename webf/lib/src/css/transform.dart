...
mixin CSSTransformMixin on RenderStyle {
  ...
  Matrix4? get transformMatrix {
    if (_transformMatrix == null && _transform != null) {
      ...
      if (_transformMatrix != null &&
          !_transformMatrix!.storage.every((e) => e.isFinite)) {
        // Real-world stylesheets occasionally produce NaN/inf (e.g. divide by
        // zero in calc, percentages against indefinite ancestors). Falling
        // through with a non-finite matrix crashes Skia in release builds, so
        // clamp to identity and drop the assertion.
  ...
  Matrix4 get effectiveTransformMatrix {
    ...
    if (!result.storage.every((double c) => c.isFinite)) {
      // Same defensive clamp as transformMatrix: never hand a non-finite
      // matrix to Flutter's paint/clip pipeline.
