  ...
  void runAnimation() {
    ...
    for (var i = 0; i < animationName.length; i++) {
      ...
      EffectTiming? options = EffectTiming(
        duration: (CSSTime.parseNotNegativeTime(duration) ?? 0).toDouble(),
        easing: timingFunction,
        delay: (CSSTime.parseTime(delay) ?? 0).toDouble(),
        ...
        direction: PlaybackDirection.values.firstWhere(
          ...
          orElse: () => PlaybackDirection.normal,
        ),
      );
      ...
      if (keyframes != null) {
        ...
        if (animation != null) {
          ...
          // Each iteration boundary fires animationiteration so JS-side
          // step counters (e.g. Tailwind `animate-bounce`, `animate-ping`)
          // observe each cycle.
          animation.oniteration = (int _) {
            target.dispatchEvent(AnimationEvent(EVENT_ANIMATION_ITERATION, animationName: name));
          };
