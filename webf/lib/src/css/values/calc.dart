...
class CSSCalcValue {
  ...
  static CSSCalcValue? tryParse(RenderStyle renderStyle, String propertyName, String propertyValue) {
    ...
    // min(a, b, ...) and max(a, b, ...) — required by Tailwind, Bootstrap, MUI.
    // Without these, responsive sizing collapses to zero.
    if (CSSFunction.isFunction(propertyValue, functionName: 'min')) {
      final fns = CSSFunction.parseFunction(propertyValue);
      if (fns.isNotEmpty && fns.first.args.isNotEmpty) {
        final values = fns.first.args
            .map((a) => CSSLength.parseLength(a.trim(), renderStyle, '${propertyName}_min_arg'))
            .toList();
        return CSSCalcValue(_MinMaxExpressionNode(values, isMin: true));
      }
    }
    if (CSSFunction.isFunction(propertyValue, functionName: 'max')) {
      final fns = CSSFunction.parseFunction(propertyValue);
      if (fns.isNotEmpty && fns.first.args.isNotEmpty) {
        final values = fns.first.args
            .map((a) => CSSLength.parseLength(a.trim(), renderStyle, '${propertyName}_max_arg'))
            .toList();
        return CSSCalcValue(_MinMaxExpressionNode(values, isMin: false));
      }
    }
    return null;
  }
...
// CSS Values Level 4: min(a, b, ...) returns the smallest, max(a, b, ...) the
// largest. Args can be lengths/percentages; if any arg is unresolved (infinity)
// we propagate that so the layout pass can retry once containers are known.
class _MinMaxExpressionNode extends CalcExpressionNode {
  final List<CSSLengthValue> values;
  final bool isMin;

  _MinMaxExpressionNode(this.values, {required this.isMin});
  ...
  double get computedValue {
    ...
    for (final v in values) {
      final cv = v.computedValue;
      // Defer when any input is unresolved (e.g. % against an indefinite parent).
      if (cv == double.infinity) return double.infinity;
      if (best == null) {
        best = cv;
      } else if (isMin ? cv < best : cv > best) {
        best = cv;
      }
    }
    return best ?? 0;
