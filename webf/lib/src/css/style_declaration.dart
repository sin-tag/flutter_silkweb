...
class CSSStyleDeclaration extends DynamicBindingObject
  ...
  // ::placeholder — styles the placeholder hint of <input>/<textarea>.
  // Tailwind: `placeholder:text-gray-400`.
  CSSStyleDeclaration? _pseudoPlaceholderStyle;
  CSSStyleDeclaration? get pseudoPlaceholderStyle => _pseudoPlaceholderStyle;
  set pseudoPlaceholderStyle(CSSStyleDeclaration? newStyle) {
    _pseudoPlaceholderStyle = newStyle;
    target?.markPlaceholderPseudoNeedsUpdate();
  }

  // ::selection — styles the user-highlighted range. We use the background
  // color via Flutter's TextSelectionTheme; foreground (color:) on inputs is
  // intentionally ignored for now (no per-character selected-text style hook
  // in Flutter's TextField without subclassing EditableText).
  CSSStyleDeclaration? _pseudoSelectionStyle;
  CSSStyleDeclaration? get pseudoSelectionStyle => _pseudoSelectionStyle;
  set pseudoSelectionStyle(CSSStyleDeclaration? newStyle) {
    _pseudoSelectionStyle = newStyle;
    target?.markSelectionPseudoNeedsUpdate();
  }
  ...
  void handlePseudoRules(Element parentElement, List<CSSStyleRule> rules) {
    ...
    if (rules.isEmpty) {
      ...
      if (pseudoPlaceholderStyle != null) {
        pseudoPlaceholderStyle = null;
      }
      if (pseudoSelectionStyle != null) {
        pseudoSelectionStyle = null;
      }
      return;
    }
    ...
    List<CSSStyleRule>? placeholderRules;
    List<CSSStyleRule>? selectionRules;
    ...
    for (CSSStyleRule style in rules) {
      ...
      if ((pseudoElementMask & kPseudoElementMaskPlaceholder) != 0) {
        (placeholderRules ??= <CSSStyleRule>[]).add(style);
      }
      if ((pseudoElementMask & kPseudoElementMaskSelection) != 0) {
        (selectionRules ??= <CSSStyleRule>[]).add(style);
      }
    }
    ...
    if (placeholderRules != null) {
      pseudoPlaceholderStyle = cascadeMatchedStyleRules(placeholderRules,
          cacheVersion: parentElement.ownerDocument.ruleSetVersion,
          copyResult: true);
    } else if (pseudoPlaceholderStyle != null) {
      pseudoPlaceholderStyle = null;
    }

    if (selectionRules != null) {
      pseudoSelectionStyle = cascadeMatchedStyleRules(selectionRules,
          cacheVersion: parentElement.ownerDocument.ruleSetVersion,
          copyResult: true);
    } else if (pseudoSelectionStyle != null) {
      pseudoSelectionStyle = null;
    }
  }
