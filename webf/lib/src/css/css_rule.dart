...
/// Heuristic for `@media (hover)` and `@media (pointer)`. We don't get the real
/// input modality from Flutter, so use the platform: mobile = touch, else hover/fine.
bool _isTouchPlatform() {
  try {
    return Platform.isAndroid || Platform.isIOS || Platform.isFuchsia;
  } catch (_) {
    // Web/unsupported platforms — assume non-touch.
    return false;
  }
}
  ...
  List<CSSRule>? getValidMediaRules(
    ...
    for (CSSMediaExpression expression in cssMediaQuery!.expressions) {
      ...
      if (expression.mediaStyle != null) {
        ...
        // orientation: portrait | landscape — extremely common in responsive
        // layouts. Without support every rule used to silently match.
        dynamic orientation = expression.mediaStyle!['orientation'];
        if (orientation != null && windowWidth != null && windowHeight != null) {
          final bool isPortrait = windowHeight >= windowWidth;
          final bool wantPortrait = orientation == 'portrait';
          conditions.add(isPortrait == wantPortrait);
          ops.add(expression.op == MediaOperator.AND);
        }
        // hover: hover | none. WebF runs on Flutter; we can't know the input
        // method reliably, so use platform heuristic — desktop has hover, the
        // touch platforms do not.
        dynamic hover = expression.mediaStyle!['hover'];
        if (hover != null) {
          final bool platformHasHover = !_isTouchPlatform();
          final bool wantHover = hover == 'hover';
          conditions.add(platformHasHover == wantHover);
          ops.add(expression.op == MediaOperator.AND);
        }
        // pointer: coarse (touch) | fine (mouse) | none. Same heuristic.
        dynamic pointer = expression.mediaStyle!['pointer'];
        if (pointer != null) {
          final bool platformIsTouch = _isTouchPlatform();
          final String want = pointer.toString();
          final bool match = (want == 'coarse' && platformIsTouch) ||
              (want == 'fine' && !platformIsTouch);
          conditions.add(match);
          ops.add(expression.op == MediaOperator.AND);
        }
        // prefers-reduced-motion. We do not currently expose a system flag, so
        // default to no-preference (animations enabled). Stylesheets that
        // request `reduce` will not match — same as the browser default for a
        // user who has not opted in.
        dynamic prefersReducedMotion =
            expression.mediaStyle!['prefers-reduced-motion'];
        if (prefersReducedMotion != null) {
          conditions.add(prefersReducedMotion == 'no-preference');
          ops.add(expression.op == MediaOperator.AND);
        }
      }
    }
