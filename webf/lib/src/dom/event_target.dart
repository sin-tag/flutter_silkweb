    ...
    if (existHandler != null) {
      ...
      try {
        ...
        for (int i = handlers.length - 1; i >= 0; i --) {
          ...
          // Sync fast-path: most listeners (Flutter-side, addEventListener wrappers
          // around sync logic) complete synchronously. Only suspend on a real Future
          // to avoid an unnecessary microtask hop per handler in deep DOM trees.
          final dynamic result = handler(event);
  ...
  Future<void> _dispatchEventInDOM(Event event) async {
    ...
    if (existHandler != null) {
      ...
      try {
        ...
        for (int i = handlers.length - 1; i >= 0; i --) {
          final handler = handlers[i];
          // Sync fast-path: skip the microtask hop when the handler completes
          // synchronously. Bubbling through a deep tree previously cost one
          // microtask per node × per handler.
          final dynamic result = handler(event);
          if (result is Future) await result;
        }
      } catch (e, stack) {
