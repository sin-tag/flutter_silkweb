...
import 'package:flutter_silkweb/src/css/query_selector.dart' as query_selector;
...
abstract class Element extends ContainerNode
  ...
  // ::placeholder pseudo update — request a widget rebuild so input elements
  // (which read pseudoPlaceholderStyle in createInputWidget) pick up the
  // new hint style on the next frame.
  bool _shouldPlaceholderPseudoNeedsUpdate = false;
  void markPlaceholderPseudoNeedsUpdate() {
    if (_shouldPlaceholderPseudoNeedsUpdate) return;
    _shouldPlaceholderPseudoNeedsUpdate = true;
    Future.microtask(_updatePlaceholderPseudo);
  }

  void _updatePlaceholderPseudo() {
    renderStyle.requestWidgetToRebuild(UpdateChildNodeUpdateReason());
    _shouldPlaceholderPseudoNeedsUpdate = false;
  }

  // ::selection pseudo update — TextSelectionTheme color changes only take
  // effect when the wrapping widget rebuilds.
  bool _shouldSelectionPseudoNeedsUpdate = false;
  void markSelectionPseudoNeedsUpdate() {
    if (_shouldSelectionPseudoNeedsUpdate) return;
    _shouldSelectionPseudoNeedsUpdate = true;
    Future.microtask(_updateSelectionPseudo);
  }

  void _updateSelectionPseudo() {
    renderStyle.requestWidgetToRebuild(UpdateChildNodeUpdateReason());
    _shouldSelectionPseudoNeedsUpdate = false;
  }
