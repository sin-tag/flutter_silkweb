...
import 'package:flutter_silkweb/src/html/canvas/canvas_path_2d.dart';
import 'package:flutter_silkweb/src/dom/intersection_observer.dart';
...
Future<void> _dispatchEventToNative(Event event, bool isCapture) async {
  ...
  if (contextId != null &&
    ...
    // Direct invocation: the underlying FFI dispatch already posts asynchronously
    // to the JS thread via the dispatcher, so wrapping it in Future.microtask just
    // adds an extra scheduler hop on every event delivery without any benefit.
